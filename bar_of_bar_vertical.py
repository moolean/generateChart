...
class bardrawer(drawer):
    ...
    def chartdrawer(self, input_dict):
        ...
        # Adding from the top matches the legend.
            ...
            y_pos = range(len(age_ratios))  # Y 轴位置
            width_tmp = np.random.uniform(0.7, 1) 
            for j, (height_new, height, label) in enumerate(reversed([*zip(age_ratios_new, age_ratios, age_labels)])):
        ...
        if bar_left:
            if needLabel < 0.7:
                if needLabel<0.35:
                    ax2.legend(bc_list2, age_labels[::-1], loc='center right', bbox_to_anchor=(0.2, 0), fontsize='medium')
                else:
                    ax2.legend(bc_list2, age_labels[::-1], loc='center right', bbox_to_anchor=(0.2, 0.5), fontsize='medium')
            elif needLabel > 0.9:
                ax1.legend(bc_list, xticklabel_list[::-1], loc='center right', bbox_to_anchor=(0, 0), fontsize='medium')
        else:
            if needLabel < 0.7:
                if needLabel<0.35:
                    ax2.legend(bc_list2, age_labels[::-1], loc='center right', bbox_to_anchor=(1.2, 0), fontsize='medium')
                else:
                    ax2.legend(bc_list2, age_labels[::-1], loc='center right', bbox_to_anchor=(1.2, 0.5), fontsize='medium')
            elif needLabel > 0.9:
...
if __name__=="__main__":
    ...
    draw = bardrawer(chart_type = "bar_of_bar_vertical", # 一定要使用规定的type名称
