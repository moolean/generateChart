...
class bardrawer(drawer):
    ...
    def chartdrawer(self, input_dict):
        ...
        for i, y_data in enumerate(data):
            ...
            # 只有一组数据，只用画一次
            if not bottom_data:
                bottom_data = [0 for i in range(len(xticklabel_list))]
            else:
                bottom_data = [bottom_data[j] + data[i-1][j] for j in range(len(xticklabel_list))]
            if bar_vertical:
                # 正常水平画表
                plt.bar(
                    r1, data[i], 
            ...
            for index, position in enumerate(bottom_data):
                position = position + y_data[index] * 0.4
                ...
                y_text.append(tmp)
        # 自动调整数字位置不重叠 avoid_self=False可以强制不改变位置
        if bar_vertical:
            adjust_text(y_text, arrowprops=dict(arrowstyle='->', color='red'), only_move = {'text': 'x'}) 
        else:
            adjust_text(y_text, arrowprops=dict(arrowstyle='->', color='red'), only_move = {'text': 'y'}) 
        ...
        if "percent" in datatype:
            if vert == 0:
                # 放置图例在图表上方外面
                plt.legend(title=legend_title, loc='lower center', bbox_to_anchor=(0.5, 1.15))
            elif vert == 1:
                # 放置图例在图表右侧外面
...
if __name__=="__main__":
    ...
    draw = bardrawer(chart_type = "stacked_bar", # 一定要使用规定的type名称
